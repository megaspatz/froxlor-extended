 ...
 *
 */
class bindextended extends DnsBase
{

	public function writeConfigs()
	{
		// tell the world what we are doing
		$this->_logger->logAction(CRON_ACTION, LOG_INFO, 'Task4 started - Rebuilding DNS-Configfiles');
		...
		$domains = $this->getDomainList();
				...
				$system_bindconf_slave_file = Settings::Get('system.bindconf_directory') . '/' . Settings::Get('system.bindconf_slave_file');
		if (! empty($domains)) {
			$bindconf_file = '# ' . $system_bindconf_file . "\n" . '# Created ' . date('d.m.Y H:i') . "\n" . '# Do NOT manually edit this file, all changes will be deleted after the next domain change at the panel.' . "\n\n";
			$bindconf_slave_file = '# ' . $system_bindconf_slave_file . "\n" . '# Created ' . date('d.m.Y H:i') . "\n" . '# Do NOT manually edit this file, all changes will be deleted after the next domain change at the panel.' . "\n\n";
	...
	private function _generateDomainSlaveConfig($domain = array())
	{
		$this->_logger->logAction(CRON_ACTION, LOG_DEBUG, 'Generating dns slave-config for ' . $domain['domain']);
